...
*/
  ...
  /**
   * How thumbnail bytes are loaded when `thumbnailRendering`
   * is `thumbnail` or `rendered`. Ignored for `wadors` and `thumbnailDirect`.
   *
...
async function fetchThumbnailWithQidoFallbackForSeries(
  ...
  const first = await fetchThumbnailObjectURL(
    thumbnailContext,
    thumbnailRendering,
    wadoRoot,
    getAuthorizationHeader
  ...
  try {
    ...
    const firstRow = instances?.[0] as Record<string, unknown> | undefined;
    const sopAttr = firstRow?.[sopInstanceUidTag] as { Value?: string[] } | undefined;
